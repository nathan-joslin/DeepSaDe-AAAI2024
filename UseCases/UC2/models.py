...
class NeuraLoanClassifier(NeuralSaDeClassifier):
    ...
    def find_initial_solution(self, K=None, bounds = None, low=-1, high=1):
        ...
        upper_bounds.extend([scaled_income, 0])
        lower_bounds.extend([0, 0])
        
        solver.add(ForAll(_X, Implies(And([And(_X[i] <= upper_bounds[i],
                                               _X[i] >= lower_bounds[i])
                                           for i in range(len(_X))]),
                                      NeuralSaDeClassifier.sumproduct(_X, W[0]) > NeuralSaDeClassifier.sumproduct(_X, W[1]))))
        ...
        for s in range(steps):
            ...
            if K is not None:
                ...
                solver.add(ForAll(_X, Implies(And([And(_X[i] <= upper_bounds[i],
                                               _X[i] >= lower_bounds[i])
                                           for i in range(len(_X))]),
                                      NeuralSaDeClassifier.sumproduct(_X, W_candidate[0]) >
                                              NeuralSaDeClassifier.sumproduct(_X, W_candidate[1]))))
    ...
    def add_knowledge_constraints(self, X, y, K=None):
        if K is not None:
            ...
            self.Hard_Constraints.append(ForAll(_X, Implies(And([And(_X[i] <= upper_bounds[i],
                                                                     _X[i] >= lower_bounds[i])
                                                                 for i in range(len(_X))]),
                                                            NeuralSaDeClassifier.sumproduct(_X, self.W[0]) >  
                                                            NeuralSaDeClassifier.sumproduct(_X, self.W[1]))))                
                
    
class LoanRegularizedModel(FFNeuralNetTorch):
    ...
    def regularized_loss(pred, target, inputs, alpha=100, K=None, loss_type='sl'):
        ...
        if K is not None:
            scaled_income = K.scale_[0] * (5000 - K.data_min_[0])
            I = torch.where((inputs.T[0] <= scaled_income) & (inputs.T[4] == 0))[0]
            if len(I) > 0:
                if loss_type == 'sl':
                    regularization_term = - torch.sum(torch.log(torch.sigmoid(pred).T[0])[I])/len(inputs)
                elif loss_type == 'sbr':
                    regularization_term = torch.tensor(1.0, requires_grad=True) - torch.sum(torch.sigmoid(pred).T[0][I])/len(inputs)
    ...
    def learn(self, X, y, learning_rate=0.001, batch_size=10, epochs=5, K=None,
              loss=None, momentum=0, alpha=10, loss_type='sl', weight_decay=0, validation_set_ratio=0.1):
        assert (loss is None)
        start = time.time()
        ...
        loss_fn = LoanRegularizedModel.regularized_loss
