...
def calculate_corrected_accuracy(X, pred, y, corrected=True):
    if corrected:
        indices_to_remove = np.where((X.T[12] == 1) & (y.T[3] == 0) & (y.T[4] == 0))[0]
...
def single_baseline_cv(X, y, train_idx=None, test_idx=None, K=None, alpha=0, fold_index=0):
    ...
    net = MusicRegularizedModel(sizes=NETWORK)
    ...
    if np.sum(np.isnan(pred_train)) == 0 and np.sum(np.isnan(pred_test)) == 0:
        accuracy_train_corrected = calculate_corrected_accuracy(X_train, pred_train, y_train)
        accuracy_test_corrected = calculate_corrected_accuracy(X_test, pred_test, y_test)

        accuracy_train = calculate_corrected_accuracy(X_train, pred_train, y_train, corrected=False)
        accuracy_test = calculate_corrected_accuracy(X_test, pred_test, y_test, corrected=False)

        total_violations_train = get_violations(pred_train, X_train)
        total_violations_test = get_violations(pred_test, X_test)
...
def single_sade_run(X, y, train_idx=None, test_idx=None, K=None, fold_index=0, folder_name='0'):
    ...
    while True:
        model = NeuralMusicClassifier(verbose=False, sizes=NETWORK, mapped_features=MAPPED_FEATURES)
        model.learn(X_train, y_train,
                    ...
                    transfer_learning_epochs=BASELINE_EPOCHS, maxsmt_optimization=MAXSMT_OPTIMIZATION
                    )
    ...
    if np.sum(np.isnan(pred_train)) == 0 and np.sum(np.isnan(pred_test)) == 0:
        ...
        return [[fold_index, get_violations(y_train, X_train), 
                 accuracy_train_corrected, accuracy_test_corrected,
                 accuracy_train, accuracy_test, total_violations_train, 
                 total_violations_test, model.runtime]], model.validation_losses
...
if __name__ == "__main__":
    ...
    if do_deepsade:
        ...
        all_deepsade_output.columns = ['fold', 'num violations training data', 'accuracy train corrected', 'accuracy test corrected', 'accuracy train', 'accuracy test',
